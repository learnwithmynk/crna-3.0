 ...
 *
 * Simplified dialog modal for logging clinical shift entries.
 * Includes: date, patient populations, medications, devices, procedures,
 * disease processes + notes, and AI smart summary.
 */
...
/**
 * Simple tag button (clean pill design, no stars)
 */
...
/**
 * Main ClinicalEntryForm component
 */
export function ClinicalEntryForm({ open, onClose, onSubmit, initialValues }) {
  ...
  // Reset form when opened/closed or initialValues change
  useEffect(() => {
    if (open) {
      if (initialValues) {
        setFormData({
        ...
        // Load custom items from initialValues if present
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Include custom items in the submission
    onSubmit({
      ...formData,
      customPopulations,
      customMedications,
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <DialogFooter className="mt-6 pt-4 border-t gap-2">
            ...
            >
              {isEditing ? 'Save Changes' : 'Log Entry +5 pts'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
...
export default ClinicalEntryForm;
